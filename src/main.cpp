...
// Copyright (c) 2015-2019 The NocNoc developers
...
#if defined(NDEBUG)
#error "NocNoc cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from NocNoc
...
void RecalculateZICHSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zNOC supply
...
{
    // NocNoc: recalculate Accumulator Checkpoints that failed to database properly
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //Track zNOC money supply in the block index
    if (!UpdateZICHSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zNOC supply for block=%s height=%d", __func__,
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // NocNoc: We use certain sporks during IBD, so check to see if they are
